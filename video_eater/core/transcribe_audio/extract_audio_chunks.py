...
def extract_audio_from_video(
    ...
    Args:
        ...
        output_file: Path to save the extracted audio (defaults to video filename with .mp3 extension)
    ...
    if output_file is None:
        output_file = video_file.replace(Path(video_file).suffix, ".mp3")
    ...
    print(f"Extracting audio from {Path(video_file).name} to {output_file}")
    ...
    return output_file, chunk_files
...
def chunk_audio_file(
    ...
    """
    chunk_folder = Path(input_file).parent / "chunks"
    ...
    with tqdm(total=num_chunks, desc=f"Chunking audio", unit="chunk") as pbar:
        for i in range(num_chunks):
            ...
            chunk_path = str(chunk_folder / chunk_filename)
    ...
    print(f"Created {len(chunk_files)} audio chunks in {chunk_folder}")
    return chunk_files
